...
Через Openstack CLI создается копия ранее созданного блочного устройства:
...
Подлинность и валидность токена - Keystone подтверждает, что токен действительно выдан им, не отозван и не просрочен (проверка подписи/записи в каталоге и TTL).

Кто это и в каком проекте - из токена читаются идентификаторы пользователя, проекта (tenant) и домена, чтобы понимать из под какой УЗ выполняется запрос.

Какие у него права - сервис смотрит список ролей в токене и сопоставляет их со своей политикой, решая разрешать ли конкретную операцию для данного пользователя в этом проекте.
...
После проверки токена через Keystone Nova дальше обращается к Glance (сервису образов).

Nova запрашивает у Glance информацию об образе, из которого нужно создать виртуальную машину (метаданные, расположение, проверка доступа), а уже потом, начинает работу с Cinder/Neutron и гипервизором.
